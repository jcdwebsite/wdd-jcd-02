...
<ul class="flex overflow-x-auto gap-3 snap-x snap-mandatory py-3 px-2">
	{#each images as { title, src, body, tag, date }, index}
		<li id={getItemId(index)} class="py-3 shrink-0 snap-center w-4/5 shadow-xl rounded-lg">
			...
			<div class="px-4">
				<h5 class="mb-2 mt-4 text-md font-bold tracking-tight text-cyan-300">{tag}</h5>
				<div class="py-2 text-2xl font-semibold tracking-tight text-cyan-300 dark:text-white">
					{title}
				</div>
				<div class="mb-3 font-normal text-cyan-300 dark:text-gray-400 leading-tight">{body}</div>
				<h5 class="mb-2 text-md tracking-tight text-cyan-200 dark:text-white">{date}</h5>
			</div>
		</li>
	{/each}
</ul>
